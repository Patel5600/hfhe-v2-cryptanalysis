...
2. **Algebraic & Composition Invariants (Phases 2–6):**
   - The cross-field embedding map $\iota: \mathbb{F}_p \hookrightarrow \mathbb{Z}/\ell\mathbb{Z}$ produces zero scalar cancellations ($R \cdot (R^{-1} \bmod p) \not\equiv 1 \pmod \ell$).
...
- Under the current evidence, the expected probability of near-term success from generic LPN search is too low to justify the required computational cost, making further brute-force compute allocation economically unviable.
