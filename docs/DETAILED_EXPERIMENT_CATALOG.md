...
This document records the complete, quantitative experiment-by-experiment catalog from artifact verification through LPN complexity assessment.

## Summary Table

| Phase | Exp ID | Script | Target Observable | Null Model | Test / Metric | N | Result / Stat | p-value | Verdict |
|---|---|---|---|---|---|---|---|---|---|
| 0 | 0.1 | `secret_ct_structure.py` | Magic, CT tags, H digest | Spec exact match | Binary parser check | 22 CTs | Match byte-for-byte | - | CLOSED |
| 0 | 0.2 | `pk_structure.py` | Decompress pk.bin, parameters | Spec exact match | Binary parser check | 1 pk | Match byte-for-byte | - | CLOSED |
| 1 | 1.1 | `cross_layer_prf.py` | Scalar weights $w$ | Uniform $\mathbb{F}_p$ | Chi-square & KS | 40,238 | Uniformity pass | >0.05 | CLOSED |
| 1 | 1.2 | `tuple_order.py` | Edge order in wire | Fisher-Yates CSPRNG | Autocorrelation | 40,238 | No sequence signal | >0.05 | CLOSED |
| 2 | 2.1 | `population_b.py` | Cross-layer $(w_0, w_1)$ | Cross-CT pairs (Pop B) | KS 2-sample | 5,000 pairs | stat=0.0263 | 0.523 | CLOSED |
| 3 | 3.1 | `public_T_distinguisher.py` | Nonce vs $w$ low bits | Permuted $(nonce, w)$ | Pearson $r$ | 40,238 | $r=-0.0218$ | 0.2317 | CLOSED |
| 4 | 4.1 | `h_rank.py` | Parity matrix $H$ GF(2) rank | Full rank (8192) | Gaussian elim GF(2) | 8192 rows | 8192 / 8192 | - | CLOSED |
| 4 | 4.2 | `powg_analysis.py` | $B=337$, $g^B$ in cyclic group | Identity element | Group theory analysis | 1 | $B = ord(g) \implies g^B=1$ | - | CLOSED |
| 5 | 5.1 | `pc_distribution.py` | Pedersen Commitments $PC$ | Uniform Ristretto point | KS 2-sample | 44 | stat=0.0348 | 0.196 | CLOSED |
| 5 | 5.2 | `cross_field_inversion.py` | $R \cdot (R^{-1} \bmod p) \bmod \ell$ | Expected 0 cancellations | Modular arithmetic trial | 100,000 | 0 / 100,000 | - | CLOSED |
| 6 | 6.1 | `character_tests.py` | Legendre $\left(\frac{-T_0 T_1}{p}\right)$ | Uniform $\pm 1$ (Ideal) | Chi-square GoF | 20,000 | $\chi^2=1.3448$ | 0.2462 | CLOSED |
| 6 | 6.2 | `sign_agreement.py` | $\Pr[\chi_2(-T_0/T_1) = \chi_2(\lambda)]$ | Bernoulli(0.5) | Exact binomial | 20,000 | Rate = 0.5000 | 0.7557 | CLOSED |
| 6 | 6.3 | `toy_ratio_experiment.py` | Matrix $\lambda$ recovery | Random baseline | Hamming distance | 1,000 | Mean = 0.5000 | - | CLOSED |
| 7 | 7.1 | `work_factor.py` | BJMM / ISD & BKW work | Practical feasibility | Complexity lower bound | LPN(4096, 1/8)| Work $> 2^{200}$ | - | ASSESSED |

---

## Detailed Experiment Records (E00 – E27)
...
**Corpus:** 1,829 weights per layer across 44 layers.
**Result:** unique across edges; zero zero-values; zero cross-object collisions.
...
**Definition:** `G = Σ sign(e) * w_e * powg_B[idx_e]`.
...
### E18 — PC distribution
REAL pairwise PC Hamming: n = 946, mean = 0.49770, SD = 0.03146, KS p = 0.1964.
Within-object PC pair: n = 22, mean = 0.49059, SD = 0.02289, p = 0.1205.
**Closed:** simple PC statistical bias.

### E19 — Fp/scalar cross-field embedding
`sc_from_fp()` embeds the 127-bit Fp representation as an integer scalar; no immediate scalar reduction occurs at that conversion. 50,000/50,000 embedding checks were exact.

### E20 — Fp-inverse versus scalar-inverse compatibility
Tested: `R * (R^{-1} mod p) == 1 mod ell`.
100,000 trials: zero hits.
**Closed:** naive cross-field cancellation.

### E21 — Wrapped ratio Legendre test
Toy N = 20,000. `χ2(-T0/T1)` vs null: χ² = 1.3448, p = 0.2462.
**Closed:** quadratic-character projection.

### E22 — Wrapped ratio sign prediction
Prediction of `χ2(lambda)` from public quotient character: rate = 50.00%, χ² = 0.0968, p = 0.7557.
...
The no-error information-set probability is `(7/8)^4096 = 2^-789.07`. This demonstrates naive error-free information-set search is infeasible, but it is **not** a formal lower bound on advanced ISD.
**Result:** no practical ISD route identified.

### E26 — BKW sample/noise tradeoff
Elimination increases noise according to `tau' = 2*tau*(1-tau)`. The available sample budget is insufficient for aggressive elimination without quickly approaching half-noise and exhausting memory/sample requirements.
